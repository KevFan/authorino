...
3. Apply an Authorino [`AuthConfig`](docsrchitecture.md#the-authorino-authconfig-custom-resource-definition-crd) custom resource
4. Obtain an authentication token and start sending requests

The full [Getting started](docsetting-started.md) page of the docs provides details for the steps above, as well as information about requirements and next steps.

Or try out our [Hello World](docsser-guides/hello-world.md) example.

For general information about protecting your service using Authorino, check out the [docs](docsEADME.md).

## Use-cases

The [User guides](docsser-guides.md) section of the docs gathers several AuthN/AuthZ use-cases as well as the instructions to implement them using Authorino. A few examples are:

- [Authentication with JWTs and OpenID Connect Discovery](docsser-guides/oidc-jwt-authentication.md)
- [Authentication with API keys](docsser-guides/api-key-authentication.md)
- [Authentication with Kubernetes SA tokens (TokenReview API)](docsser-guides/kubernetes-tokenreview.md)
- [Authentication with X.509 certificates and mTLS](docsser-guides/mtls-authentication.md)
- [Authorization with JSON pattern-matching rules (e.g. JWT claims, request attributes, etc)](docsser-guides/json-pattern-matching-authorization.md)
- [Authorization with Open Policy Agent (OPA) Rego policies](docsser-guides/opa-authorization.md)
- [Authorization using the Kubernetes RBAC (rules stated in K8s `Role` and `RoleBinding` resources)](docsser-guides/kubernetes-subjectaccessreview.md)
- [Authorization using auth metadata fetched from external sources](docsser-guides/external-metadata.md)
- [OIDC authentication and RBAC with Keycloak JWTs](docsser-guides/oidc-rbac.md)
- [Injecting auth data into the request (HTTP headers, Wristband tokens, rate-limit metadata, etc)](docsser-guides/injecting-data.md)
- [Authorino for the Kubernetes control plane (aka Authorino as ValidatingWebhook service)](docsser-guides/validating-webhook.md)
...
<details markdown="1">
  ...
  The [Architecture](docsrchitecture.md) section of the docs covers details of protecting your APIs with Envoy and Authorino, including information about topology (centralized gateway, centralized authorization service or sidecars), deployment modes (cluster-wide reconciliation vs. namespaced instances), an specification of Authorino's [`AuthConfig`](docsrchitecture.md#the-authorino-authconfig-custom-resource-definition-crd) Custom Resource Definition (CRD) and more.

  You will also find in that section information about what happens in request-time (aka Authorino's [Auth Pipeline](docsrchitecture.md#the-auth-pipeline-aka-enforcing-protection-in-request-time)) and how to leverage the [Authorization JSON](docsrchitecture.md#the-authorization-json) for writing policies, dynamic responses and other features of Authorino.
</details>
...
For a detailed description of the features above, refer to the [Features](docseatures.md) page.
...
<details markdown="1">
  ...
  Authorino also provides a second interface for [raw HTTP authorization](docsrchitecture.md#raw-http-authorization-interface), suitable for using with Kubernetes ValidatingWebhook and other integrations (e.g. other proxies).

  The only attribute of the authorization request that is strictly required is the host name. (See [Host lookup](docsrchitecture.md#host-lookup) for more information.) The other attributes, such as method, path, headers, etc, might as well be required, depending on each `AuthConfig`. In the case of the gRPC [`CheckRequest`](https://pkg.go.dev/github.com/envoyproxy/go-control-plane/envoy/service/auth/v3?utm_source=gopls#CheckRequest) method, the host is supplied in `Attributes.Request.Http.Host` and alternatively in `Attributes.ContextExtensions["host"]`. For raw HTTP authorization requests, the host must be supplied in `Host` HTTP header.
...
<details markdown="1">
  ...
  Authorino is not an identity broker either. It can verify access tokens from multiple trusted sources of identity and protocols, but it will not negotiate authentication flows for non-authenticated access requests. Some tricks nonetheless can be done, for example, to [redirect unauthenticated users to a login page](docsser-guides/deny-with-redirect-to-login.md).
...
<details markdown="1">
  ...
  No, you do not. However, if you are comfortable with [Rego](https://www.openpolicyagent.org/docs/latest/policy-language/) from Open Policy Agent (OPA), there are some quite interesting things you can do in Authorino, just as you would in any OPA server or OPA plugin, but leveraging Authorino's [built-in OPA module](docseatures.md#open-policy-agent-opa-rego-policies-authorizationopa) instead. Authorino's OPA module is compiled as part of Authorino's code directly from the Golang packages, and imposes no extra latency to the evaluation of your authorization policies. Even the policies themselves are pre-compiled in reconciliation-time, for fast evaluation afterwards, in request-time.

  On the other hand, if you do not want to learn Rego or in any case would like to combine it with declarative and Kubernetes-native authN/authZ spec for your services, Authorino does complement OPA with at least two other methods for expressing authorization policies – i.e. [pattern-matching authorization](docseatures.md#pattern-matching-authorization-authorizationpatternmatching) and [Kubernetes SubjectAccessReview](docseatures.md#kubernetes-subjectaccessreview-authorizationkubernetessubjectaccessreview), the latter allowing to rely completely on the Kubernetes RBAC.
...
<details markdown="1">
  ...
  Additionally, when enabling the request body passed in the payload to Authorino, parsing of the content should be of concern as well. Authorino provides easy access to attributes of the HTTP request, parsed as part of the [Authorization JSON](docsrchitecture.md#the-authorization-json), however the body of the request is passed as string and should be parsed by the user according to each case.

  Check out Authorino [OPA authorization](docseatures.md#open-policy-agent-opa-rego-policies-authorizationopa) and the Rego [Encoding](https://www.openpolicyagent.org/docs/latest/policy-reference/#encoding) functions for options to parse serialized JSON, YAML and URL-encoded params. For XML transformation, an external parsing service connected via Authorino's [HTTP GET/GET-by-POST external metadata](docseatures.md#http-getget-by-post-metadatahttp) might be required.
</details>
...
<details markdown="1">
  ...
  In terms of scale, Authorino `AuthConfig`s should grow proportionally to the number of protected services, virtually limited by nothing but the Kubernetes API data storage, while [namespace division](docsrchitecture.md#cluster-wide-vs-namespaced-instances) and [label selectors](docsrchitecture.md#sharding) help adjust horizontally and keep distributed.
...
<details markdown="1">
  ...
  You can, but you shouldn't. Check out instead [Limitador](https://github.com/kuadrant/limitador), for simple and efficient global rate limiting. Combine it with Authorino and Authorino's support for [Envoy Dynamic Metadata](docseatures.md#envoy-dynamic-metadata) for authenticated rate limiting.
</details>
...
If you are interested in contributing to Authorino, please refer to the [Developer's guide](docsontributing.md) for info about the stack and requirements, workflow, policies and Code of Conduct.
